...
export function setDraggableRegion(
  ...
  return new Promise((resolve: any, reject: any) => {
    const draggableRegion: HTMLElement =
      domElementOrId instanceof Element
        ? domElementOrId
    ...
    draggableRegions.set(draggableRegion, {
      pointerdown: startPointerCapturing,
      pointerup: endPointerCapturing,
    });
    ...
    async function onPointerMove(evt: PointerEvent) {
      if (isPointerCaptured) {
        ...
        await move(evt.screenX - initialClientX, evt.screenY - initialClientY);

        return;
      }

      // Add absolute drag distance
      absDragMovementDistance = Math.sqrt(
        evt.movementX * evt.movementX + evt.movementY * evt.movementY,
      );
      ...
      if (absDragMovementDistance >= 10) {
        // TODO: introduce constant instead of magic number?
